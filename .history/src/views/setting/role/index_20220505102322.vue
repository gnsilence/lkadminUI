<template>
  <el-container>
    ...
    <el-main class="nopadding">
      ...
      >
        <el-table-column hide label="角色名称" align="left" show-overflow-tooltip>
          <template #default="scope">{{ scope.row.name }}</template>
        </el-table-column>
        <el-table-column label="角色描述" align="left" show-overflow-tooltip>
          <template #default="scope">{{ scope.row.remark }}</template>
        </el-table-column>
        <el-table-column property="sort" label="排序" width="120" align="right" />
        <el-table-column label="操作" width="200" align="center">
          <template #default="scope">
            ...
            <el-dropdown>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="a" @click="handleCommand('a',scope.row)">授权菜单</el-dropdown-item>
                  <el-dropdown-item command="b" @click="handleCommand('b',scope.row)">授权数据</el-dropdown-item>
                  <el-dropdown-item
    ...
    <el-container v-loading="drawerLoading">
      <el-main>
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item ref="Name" label="角色名称:" prop="Name">
            <el-input v-model="ruleForm.Name" maxlength="10" placeholder="请输入角色名称（最多10汉字字符）" />
          </el-form-item>
          <el-form-item ref="Sort" label="排序:" prop="Sort">
            ...
            />
            <div class="el-form-item-msg">本列表中及前端调用的展示顺序（编辑范围：1~{{ maxNum }}）</div>
            <!-- <el-input v-model="ruleForm.Sort" placeholder="请输入整数" /> -->
          </el-form-item>
  ...
  <AuthorizedMenu ref="authorizedMenu" @onSearch="onSearch" />
  <AuthorizationData ref="authorizationData" @onSearch="onSearch" />
</template>
...
import { page, addPro, deleterRole, editPro, defSort } from '@/api/setting/role/idnex.js'
import { role_rules } from '@/utils/rules/setting/role/index.js'
import AuthorizedMenu from '@/views/setting/role/components/AuthorizedMenu'
import AuthorizationData from '@/views/setting/role/components/AuthorizationData'

export default {
  components: { Plus, Search, ArrowDown, AuthorizedMenu, AuthorizationData },
  setup() {
