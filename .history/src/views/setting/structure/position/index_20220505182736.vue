 ...
 * @LastEditors: gongnan
 * @LastEditTime: 2022-05-05 18:24:31
...
import { sysPosPage,sysPosAdd,sysPosEdit,sysPosDelete } from '@/api/setting/structure/position.js'
...
export default {
  ...
  setup() {
    ...
    const handleCommand = async (command, row) => {
      var data =_this.$TOOL.objCopy(row)
      if (command === 'a') {
		  data.status=1
       console.log('%c⧭', 'color: #917399', data)
       await SetPositionStatus(data)
      } else if (command === 'b') {
		  row.status=0
        await SetPositionStatus(row)
      } else if (command === 'c') {
        handleDelete(row)
      }
    }
	...
	const SetPositionStatus=async (row)=>{
		const res = await sysPosEdit(row)
		if (res.code !== 200) {
            ElMessage.error(res.message || '启用岗位失败，请重试！')
            return
          }
	}
    // 编辑
    ...
    const handleDelete = (row) => {
      ElMessageBox.confirm('确定删除岗位信息吗？')
        .then(async() => {
          const res = await sysPosDelete({ Id: row.id })
          if (res.code !== 200) {
      ...
      ruleFormRef.value.validate(async(valid) => {
        if (valid) {
          ...
          if (ruleForm.value.Id) {
            const res = await sysPosEdit(ruleForm.value)
            ...
            const res = await sysPosAdd(ruleForm.value)
            drawerLoading.value = false
            if (res.code !== 200) {
              ElMessage.error(res.message || '保存失败，请重试！')
              return
