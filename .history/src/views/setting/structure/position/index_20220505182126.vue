 ...
 * @LastEditors: gongnan
 * @LastEditTime: 2022-05-05 18:21:26
...
export default {
  ...
  setup() {
    ...
    const handleCommand = async (command, row) => {
      ...
      if (command === 'a') {
       await SetPositionStatus(1)
      } else if (command === 'b') {
        await SetPositionStatus(0)
      } else if (command === 'c') {
        handleDelete(row)
      }
    }
	...
	const SetPositionStatus=async (status)=>{
		const res = await sysPosEdit({status:status})
		if (res.code !== 200) {
