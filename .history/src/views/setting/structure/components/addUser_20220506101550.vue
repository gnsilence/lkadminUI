			...
			<el-tab-pane name="emp" label="员工信息">
				...
				>
					...
					<el-form-item label="直属岗位:" prop="limitTime">
						 <el-select class="width-100" v-model="Empform.PosIdList" multiple clearable filterable remote @change="contractServiceMansChange" placeholder="请选择岗位">
          <el-option v-for="item in serviceMansOptions" :key="item.value" :label="item.label" :value="item.value">
		  {{item.name}}
            </el-option>
          </el-select>
					</el-form-item>
...
<template v-if="tabname === 'emp'" #footer>
	...
	<el-button type="primary" :disabled="Empform.orgId.length == 0" :loading="isSaveing" @click="clkSubmit">确定</el-button>
</template>
	</scDialog>
</template>
