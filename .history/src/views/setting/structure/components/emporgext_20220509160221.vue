 ...
 * @LastEditors: gongnan
 * @LastEditTime: 2022-05-09 16:02:21
				...
				<div class="flex mb15 justify-space-between align-items productTableItem" v-for="(val, index) in emporgLists.emp">
					...
					<div class="wp-150">
						<el-form-item :prop="'emp.' + index + '.PosId'" :rules="rules.price">
							<el-select v-model="val.PosId" filterable clearable placeholder="请选择岗位" style="width:100%" @clear="clearSelect" @change="chgSelect($event)">
								<el-option v-for="(item,t_index) in positionList" :key="item.value" :label="item.label" :value="{
									value:item.value,
									name:item.label
